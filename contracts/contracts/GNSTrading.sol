...
import "../libraries/TradeUtilsV7.sol";
...
contract GNSTrading is Initializable, Delegatable, IGNSTrading {
    ...
    function openTrade(
        ...
        uint256 levPosDai = t.positionSizeDai * t.leverage;
        uint256 levPosUsd = aggregator.getUsdNormalizedValue(levPosDai);
        require(
            storageT.openInterestDai(t.pairIndex, t.buy ? 0 : 1) + levPosDai <=
                borrowingFees.getCollateralPairMaxOi(t.pairIndex),
            "ABOVE_PAIR_MAX_OI"
        );
        require(borrowingFees.withinMaxGroupOi(t.pairIndex, t.buy, levPosDai), "ABOVE_GROUP_MAX_OI");
        require(levPosUsd >= multiCollatDiamond.pairMinLevPosUsd(t.pairIndex), "BELOW_MIN_POS");
        ...
        (uint256 priceImpactP, ) = multiCollatDiamond.getTradePriceImpact(0, t.pairIndex, t.buy, levPosUsd);
        require(priceImpactP * t.leverage <= MAX_OPEN_NEGATIVE_PNL_P, "PRICE_IMPACT_TOO_HIGH");
    ...
    function closeTradeMarket(uint256 pairIndex, uint256 index) external notContract notDone {
        ...
        uint256 orderId = storageT.priceAggregator().getPrice(
            ...
            (t.initialPosToken * i.tokenPriceDai * t.leverage) / collateralConfig.precisionDelta / PRECISION,
            ChainUtils.getBlockNumber()
        );
    ...
    function executeNftOrder(uint256 packed) external notContract notDone {
        ...
        uint128 collateralPrecisionDelta = collateralConfig.precisionDelta;
                ...
                if (t.sl > 0) {
                    uint256 liqPrice = borrowingFees.getTradeLiquidationPrice(
                        IGNSBorrowingFees.LiqPriceInput(
                            ...
                            t.buy,
                            (t.initialPosToken *
                                storageT.openTradesInfo(t.trader, t.pairIndex, t.index).tokenPriceDai) /
        ...
        if (isOpenLimit) {
            ...
            uint256 _leveragedPosUsd = storageT.priceAggregator().getUsdNormalizedValue(_leveragedPosDai);
            (uint256 priceImpactP, ) = multiCollatDiamond.getTradePriceImpact(0, l.pairIndex, l.buy, _leveragedPosUsd);

            require(priceImpactP * l.leverage <= MAX_OPEN_NEGATIVE_PNL_P, "PRICE_IMPACT_TOO_HIGH");
    ...
    function _getPriceNftOrder(
        ...
        orderId = storageT.priceAggregator().getPrice(
            pairIndex,
